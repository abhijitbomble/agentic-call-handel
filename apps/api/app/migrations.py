...
def ensure_program_policy_columns() -> None:
...
def backfill_program_policy(db: Session | None = None) -> None:
    session = db or SessionLocal()
    close_session = db is None
    try:
        programs = session.query(ClientProgram).all()
        changed = False
        for program in programs:
            ...
            if not policy:
                ...
                if isinstance(program.handoff_policy, dict) and program.handoff_policy:
                    policy["handoff_policy"] = {**policy.get("escalation_policy", {}), **program.handoff_policy}
...
def ensure_program_policy_schema() -> None:
    ensure_program_policy_columns()
